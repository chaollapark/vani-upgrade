...
if (($func == "nav_catg") || ($func == "nav_page")) {
  ...
  //$target = remove_accents(@$_GET['target']);
  if (!in_array($params->sort,['titl','petl','lang'])) return 0;
  $ofs = @$_GET['ofs'];
  $lim = @$_GET['lim'];
  $page = @$_GET['page'];
...
function get_titl($type, $ofs, $lim, $page, $params) {
  ...
  $bind_params[] = $ofs; $bind_types .= 'i';
  $bind_params[] = $lim; $bind_types .= 'i';
      ...
      for ($i = 0; $i < $cnt; $i++) {
        ...
        switch (true) {
          case (($op == "contains_any") || ($op == "contains_all")):
            $where .= "$field LIKE ?"; $bind_params[] = "%$words[$i]%"; $bind_types .= 's'; break;
          case ($op == "starts_any"):
            $where .= "$field LIKE ?"; $bind_params[] = "$words[$i]%"; $bind_types .= 's'; break;
          case ($op == "ends_any"):
            $where .= "$field LIKE ?"; $bind_params[] = "%$words[$i]"; $bind_types .= 's'; break;
