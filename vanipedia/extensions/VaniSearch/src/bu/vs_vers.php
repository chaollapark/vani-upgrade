...
function get_nav_line($params) {
  ...
  $sql =
    "SELECT line_text orig, min(l.vers_ref) vers FROM line l ".$join."WHERE $where ".
    "group by line_text order by $order";
...
function get_line($ofs, $lim, $page, $params) {
  global $mysqli;
  if (!$params->target) return "";
  $target = $mysqli->real_escape_string($params->target);
  $target = preg_replace("/\s+/", " ",$target);
  ...
  $sql =
    "SELECT line_text orig, min(l.vers_ref) vers FROM line l $join WHERE $where ".
    "group by line_text order by $order LIMIT $ofs,$lim ";
  $result = $mysqli->query($sql);
  $table = "";
  ...
  while ($values = $result->fetch_row()) {
    ...
    $sql_2 =
      ...
      "line_text = '$esc_text' ". // maybe compare with utf8mb4_bin collation? (separate field?)
      "ORDER BY l.vers_ref ";
    $refs = "";
...
function get_nav_trns($params) {
  ...
  regexp_translit_prepare ($params->ds,$params->op,$target,$transliterator,$tl_target,$beg,$end,$words);
  $sql =
    "SELECT tran_text tran, vers_ref vers FROM tran ".
    "INNER JOIN vers using(vers_id) ".
...
function get_trns($ofs, $lim, $page, $params) {
  ...
  regexp_translit_prepare ($params->ds,$params->op,$target,$transliterator,$tl_target,$beg,$end,$words);
  $sql =
    "SELECT tran_text tran, vers_ref vers, vers_url FROM tran ".
    "INNER JOIN vers USING(vers_id) WHERE $where ".
    "order by $order LIMIT $ofs,$lim ";
  $result = $mysqli->query($sql);
  $table = "";
  $idx = ($page - 1) * $params->rpp + 1;
  while ($values = $result->fetch_row()) {
...
function syno_sql_1 (&$sea_orig,&$sea_tran,$ofs,$lim,$params) {
  ...
  $sea_orig = $mysqli->real_escape_string($params->sea_orig);
  $sea_orig = preg_replace("/\s+/", " ",$sea_orig);
  $sea_tran = $mysqli->real_escape_string($params->sea_tran);
  $sea_tran = preg_replace("/\s+/", " ",$sea_tran);
  $order = $params->sort.[" asc"," desc"][$params->dir]; // todo
  ...
  $sql =
    ...
    " group by syno_orig, syno_tran ORDER BY $order".($lim ? " LIMIT $ofs,$lim " : "");
  return $sql;
}
...
function syno_sql_2 ($raw_orig,$raw_tran,$params) {
  ...
  $esc_orig = $mysqli->real_escape_string($raw_orig);
  $esc_tran = $mysqli->real_escape_string($raw_tran);
  $sql =
      ...
      ($params->part ? "(part_id = $params->part) AND ": "").
      ($params->chap ? "(chap_id = $params->chap) AND ": "").
      "syno_orig = '$esc_orig' AND syno_tran = '$esc_tran' ".
      "ORDER BY v.vers_ref ";
  return $sql;
}
...
//  if ($collate) $field = $field." COLLATE utf8mb4_unicode_ci";
  ...
  switch (true) {
    ...
    case (($op == "contains_any") || ($op == "starts_any") || ($op == "ends_any") || ($op == "contains_all")):
      ...
      $where .= ")";
      break;
    case ($op == "shorter_than"):
      if (is_numeric($target)) $where = "CHAR_LENGTH($field) < $target ";
      else $where = "0"; break;
    case ($op == "longer_than"):
      if (is_numeric($target)) $where = "CHAR_LENGTH($field) > $target ";
      else $where = "0"; break;
