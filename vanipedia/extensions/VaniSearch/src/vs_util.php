...
function get_matches_titl($str, $nspace, $lang, $vp, $vq, $vs, $vm) {
  ...
  $params[] = "$str%"; $types .= 's'; /* LIKE */
  $and_lang = "";
  if ($lang) {
    $and_lang = " and lang_code = ? ";
    $params[] = $lang; $types .= 's';
  }
  ...
  $sql =
    ...
    " where tokn_text collate utf8mb4_unicode_ci like ? ".
...
function get_fdbk_text($fdbk_id) {
  ...
  $sql = "select fdbk_text from fdbk where fdbk_id = ? ";
  
  $stmt = $mysqli->prepare($sql);
  $stmt->bind_param('i', $fdbk_id);
  $stmt->execute();
  $result = $stmt->get_result();
...
function insert_fdbk($params) {
  ...
  $stmt = $mysqli->prepare($sql);
  $stmt->bind_param('ississs', $params->cntr_id, $params->name, $params->email, $params->rating, $params->favorite, $text, $ip);
  $stmt->execute();
