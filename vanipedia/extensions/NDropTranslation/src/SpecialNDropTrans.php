...
class SpecialNDropTrans extends SpecialPage {
  ...
  function upd_setup () {
      ...
      $id = $user->getId();
      if ($id == 0) {
        echo '{"error":"You are not logged in"}'; return;
      }
      //$rights = $user->getRights();
      //if (!in_array("edit", $rights)) {
      if(!$user->isAllowed("edit")) {
        echo '{"error":"You do not have edit-permission: please consult an administrator"}'; return;
      }      
  ...
  function db_connect () {
    $mysqli = new mysqli("localhost",$this->config['DB_USER'],$this->config['DB_PASS'],"vp_translate");
