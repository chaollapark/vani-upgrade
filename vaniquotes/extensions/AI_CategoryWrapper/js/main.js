...
async function initPage(parentId) {
  ...
  await ctgLogic.genWrapRecords();
  ctgLogic.getCategories(true);
}
...
//    this.sIndex = 0;
  ...
  fillTbody() {
    let idx; let cel; let href; let instance;
    ...
    this.creator.data.forEach(rec => {
      ...
      this.creator.columns.forEach(col => {
        ...
        else if (col.type == "lnk") {
          href = `https://${sel_petl.value}.org/wiki/Category:${rec[col.name].replaceAll(" ","_")}`;
          instance = new HtmlA({href, target:"_blank", text:rec[col.name], cls:"mylink"});
          cel = this.addCell(row,"",col.cls,col.width);
  ...
  async toggleInclude(e,cat_id) {
    ...
    if (! await this.creator.updateOneInclude(cat_id,nextValue)) return;
  ...
  toggleSort(idx, obj) {
    ...
    this.creator.getCategories(true);
  }
