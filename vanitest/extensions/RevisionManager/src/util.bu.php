...
function sql_pages($petal, $nspace, $type, $rpp, $ofs, $sort) {
  ...
  if ($type == "one")
    $sql =
      ...
      "where page_namespace = $nspace $order";
  else if ($type == "all")
    $sql =
      ...
      "where page_namespace = $nspace ".
      "group by page_id, page_title $order";
  return $sql;    
}
...
function get_count($petal, $nspace, $type) {
  ...
  $sql = sql_count($petal, $nspace, $type);
  $result = $mysqli->query($sql);
...
function del_history($range, $petal, $del_until, $preserve_first) {
