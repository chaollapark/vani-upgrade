...
function sqlNDrops ($year, $loca, $lang_code, $ndsp) {
  ...
  if ($ndsp) $sql = 
    ...
    " and page_title like concat('".$lang_code."/',q.prefix,'%') ".
    " and cl_to = '".$ndsp."') ";
  return $sql;
}
...
function getTransData($page_id,$lang_code,$lang_name,$english) {
  ...
  if ($id) {
    ...
    $title = str_replace("_"," ",getTransTitle($page_id,$lang_code));
    $text = ""; 
...
function getTransTitle ($page_id,$lang_code) {
  global $mysqli;
  $sql = "select vp_translate.nectar_drop('$lang_code',$page_id) from dual";
  $fields = $mysqli->query($sql)->fetch_row();
  return $fields[0];
}
...
function getSrefTrans($lang_code,$text) {
  ...
  $sql =
    ...
    "inner join vp_translate.tran t on t.tran_lang = '$lang_code' and t.tran_code = o.tran_code ".
...
function getTranText($idx,$lang, $code) {
  ...
  $sql =
    ...
    "where tran_lang = '$lang' and tran_code= '$code'";
  $result = $mysqli->query($sql);
  if ($result->num_rows == 0) return "";
  else {
    $fields = $result->fetch_row();
...
function getSetupData($lang_code) {
  ...
  $sql =
    ...
    "left join vp_translate.tran t on o.tran_code = t.tran_code and t.tran_lang = '".$lang_code."' ".
